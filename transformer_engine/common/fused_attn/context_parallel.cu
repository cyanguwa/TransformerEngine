                                    ...
                                    int chunk_size)
{
    ...
    for (int j = 0; j < batch; ++j)
    {
        ...
        int num_chunks  = (d_cu_seqlens_padded_j_1 - d_cu_seqlens_padded_j + (chunk_size - 1)) / chunk_size;
    ...
    if (!found)
    {
        ...
        int32_t out_seq  = ((pos_id > (seq_len / chunk_size)) ? seq_len : chunk_size * pos_id) + seq_start;
        int32_t out_pad  = ((pos_id > (pad_len / chunk_size)) ? pad_len : chunk_size * pos_id) + pad_start;
...
__global__ void thd_chunkify_p2p_kernel(
    const int* __restrict__ d_cu_seqlens,
    const int* __restrict__ d_cu_seqlens_padded,
    int* __restrict__ d_out_cu_seqlens,
    int* __restrict__ d_out_cu_seqlens_padded,
    int batch,
    ...
    int cp_size)
{
    ...
    if (pos_id == 0) {
        ...
        out_pad_len = first_chunk_size_1 + num_chunks_1 * chunk_size + middle_chunk_1 + middle_chunk_2 + num_chunks_2 * chunk_size;
    } else {
...
__global__ void thd_seq_tweak_below_diag_kernel(
    ...
    const int* __restrict__ cu_seqlens_kv_halfs,
    const int* __restrict__ cu_seqlens_padded,
    int* __restrict__ q_chunks,
    int* __restrict__ kv_chunks,
    int* __restrict__ q_pads,
    int* __restrict__ kv_pads,
    int  cp_rank_q,
    int  cp_rank_kv,
    int  cp_size,
    int  chunk_size,
    int  batch)
{
    ...
    const int32_t last_kv_chunk_len = min(
        half_seq_len -
        (last_kv_chunk_id * chunk_size - cp_rank_kv * half_seq_len),
        half_seq_len);

    const int32_t first_half_id  = (cp_rank_q * half_seq_len) / chunk_size;
    const int32_t second_half_id = ((2 * cp_size - cp_rank_q - 1) * half_seq_len)
                                     / chunk_size;

    const int32_t first_half_len = min(
        half_seq_len,
        (first_half_id + 1) * chunk_size - cp_rank_q * half_seq_len);

    const int32_t second_half_len = min(
        seq_len_q,
        half_seq_len +
        (second_half_id + 1) * chunk_size -
        (2 * cp_size - 1 - cp_rank_q) * half_seq_len);

    const bool take_nothing       = (last_kv_chunk_id != first_half_id);
    const bool take_first_half_q  = (!take_nothing) &&
                                    (last_kv_chunk_id != second_half_id);
    const bool take_second_half_q = (!take_nothing) && (!take_first_half_q);

    int32_t q_seq_len = 0;
    if (take_first_half_q)      q_seq_len = first_half_len;
    else if (take_second_half_q) q_seq_len = second_half_len;
    ...
    int  batch)
{
    ...
    const int32_t first_q_chunk_len = min(
        seq_len_q_half,
    ...
    const int32_t first_half_kv_last_chunk_len = min(
        seq_plus_pad,
        half_seq_len + first_half_kv_last_el_id_in_chunk + 1);

    const int32_t second_half_kv_last_el_id_in_chunk = second_half_kv_last_el_total_id % chunk_size;
    const int32_t second_half_kv_last_chunk_len = min(
        half_seq_len,
        second_half_kv_last_el_id_in_chunk + 1);

    // 3. Decide which half(s) we actually take
    const bool take_nothing          = (first_q_chunk_id != second_half_kv_last_chunk_id);
    const bool take_second_half_kv   = (!take_nothing) && (first_q_chunk_id != first_half_kv_last_chunk_id);
    ...
    if (take_second_half_kv)      kv_seq_len = second_half_kv_last_chunk_len;
    else if (take_first_half_kv)  kv_seq_len = first_half_kv_last_chunk_len;
...
void thd_chunkify(
  ...
  Tensor &out_cu_seqlens,
  Tensor &out_cu_seqlens_padded,
  int batch,
  int output_len,
  int chunk_size,
  cudaStream_t stream
...
void nvte_cp_thd_chunkify(const NVTETensor &cu_seqlens, const NVTETensor &cu_seqlens_padded,
  ...
  context_parallel::thd_chunkify(*convertNVTETensorCheck(cu_seqlens),
